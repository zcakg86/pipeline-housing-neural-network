...
async function loadSales() {
  ...
  const colorModel = variable === 'predicted_price_lightgbm' ? 'lightgbm'
    : variable === 'predicted_price_neural' ? 'neural'
    : document.getElementById('h3Model').value;
  ...
  const gj = await fetchLayerJson('sales',
  ...
  L.geoJSON(gj, {
    ...
    onEachFeature: (f, layer) => {
      ...
      const attnRow = (label, val) =>
        val != null ? `${label}: ${(val*100).toFixed(1)}%<br>` : '';
      layer.bindPopup(
        ...
        `95% CI: ±$${Math.round((p.avgPredStd ?? 0) * 1.96).toLocaleString()} (±${((p.avgPredCvPct ?? 0) / 2).toFixed(1)}% of price)<br>` +
...
function buildLegend(variable, values, anchors) {
  ...
  legendControl.onAdd = () => {
    ...
    const titles = {
      ...
      sale_price: 'Average Sale Price', sqft: 'Average Sqft', num_sales: 'Sales',
      pred_std: 'Prediction Uncertainty', pred_cv_pct: 'Uncertainty Width'
    };
    const modelSuffix = variable === 'pct_error'
      ? ` — ${document.getElementById('h3Model').selectedOptions[0].textContent}` : '';
    const lo = anchors ? anchors.p10 : pct(values, 5);
    const hi = anchors ? anchors.p90 : pct(values, 95);
    const format = value => [
      'sale_price', 'predicted_price_neural', 'predicted_price_lightgbm', 'pred_std'
    ].includes(variable)
    ...
    const palette = variable === 'pct_error'
      ...
      : (['sqft', 'pred_std', 'pred_cv_pct'].includes(variable)
...
function pointVariableValue(props, variable) {
  switch (variable) {
    case 'sale_price':      return props.salePrice    ?? props.listPrice ?? 0;
    case 'predicted_price_neural': return props.predictedPrice ?? props.neuralPredictedPrice ?? 0;
    case 'predicted_price_lightgbm': return props.lightgbmPredictedPrice ?? 0;
    ...
    case 'attn_year':       return props.attnYear      ?? 0;
    case 'attn_week':       return props.attnWeek      ?? 0;
    ...
    default:                return props.pctError      ?? 0;
  }
}
